...
CREATE TABLE BANK_T (
    ...
    CONSTRAINT PK_BANK PRIMARY KEY(BANK_CODE) --기본키에 이름 부여하는 방법
);
...
CREATE TABLE CUSTOMER_T (
    ...
    CUST_AGE   NUMBER(3)          CHECK(CUST_AGE >= 0 AND CUST_AGE <= 120),
                                  -- CHECK(CUST_AGE BETWEEN 0 AND 120), 나이가 0~120
...
/*
    ...
    4. 칼럼 이름 바꾸기   : ALTER TABLE 테이블명 RENAME COLUM  기존칼럼명 TO 신규칼럼명
...
-- 3. CUSTOMER_T의 나이(CUST_AGE) 칼럼을 삭제하시오.
      ALTER TABLE CUSTOMER_T DROP COLUM CUST_AGE;

-- 4. CUSTOMER_T의 연락처(CUST_PHONE) 칼럼명을 CUST_TEL로 수정하시오.
      ALTER TABLE CUSTOMER_T RENAME COLUM CUST_PHONE TO CUST_TEL;

-- 5. CUSTOMER_T에 등급(GRADE) 칼럼을 추가하시오. 'VIP', 'GOLD', 'SILVER', 'BRONZE' 중 하나의 값을 가지도록 설정하시오.
      --ALTER TABLE CUSTOMER_T ADD GRADE CHECK('VIP' OR 'GOLD' OR 'SILVER' OR 'BRONZE');
...
/*
    ...
    2. FK
        1) 추가     : ALTER TABLE 자식테이블명 ADD CONSTRAINT     제약조건명  FORIEGN KEY(칼럼명) REFERENCES 부모테이블명(참조할칼럼명) [옵션]
        2) 삭제     : ALTER TABLE 테이블명     DROP CONSTRAINT    제약조건명
        3) 비활성화 : ALTER TABLE 테이블명     DISABLE CONSTRAINT 제약조건명
        4) 활성화   : ALTER TABLE 테이블명     ENALBE CONSTRAINT  제약조건명
*/
...
--SELECT * FROM USER_CONSTRAINTS; --제약조건보기
