/*
    ...
    3. 참조 무결성을 가져야 한다.
        ...
        2) 참조 무결성이 위배되는 경우를 대비해 추가 옵션을 사용할 수 있다.
           ...
           (2)ON DELETE CASCADE : 부모키가 삭제되는 경우 자식키를 가진 행row 전체를 삭제한다.
    4. 형식
        부모 테이블 - 자식 테이블 
        부모 키(parent key) - 자식 키(child key)
        기본키/UNIQUE - 외래키
*/
...
/*
    일대다 관계
    1. 2개의 테이블을 관계 짓는 사장 흔한 관계이다.
    2. 1 : M 
       PK : FK
       UQ
       부모 : 자식
    3. 반드시 부모 테이블을 먼저 만들어야 한다.
    4. 반드시 자식 테이블부터 삭제해야 한다.
    5. 테이블 생성/삭제 규칙
        생성과 삭제 순서는 항상 반대이다. (생성 ㄱ-ㄴ-ㄷ/삭제 ㄷ-ㄴ-ㄱ)
*/


-- 1. 제품 테이블 (기본키에 들어있는 참조용 테이블 먼저 작성) (parent key)
