/*
    ** DML
    ...
    4. 트랜잭션 처리가 필요하다.
    5. 종류
        1) INSERT INTO .. VALUES ..    : 행 삽입 
        2) UPDATE .. SET .. WHERE ..   : 행 수정
        3) DELETE FROM .. WHERE ..     : 행 삭제
*/
...
/*
    ** 트랜잭션 (TRANSACTION)
    ...
    3. 작업의 저장을 의미하는 COMMIT과 작업의 취소를 의미하는 ROLLBACK이 필요하다. (TCL : Transaction Control Language)
    4. 예시 : 은행 이체
        1) 내 통장에서 돈 빼기
        2) 상대 통장으로 돈 넣기
*/
...
CREATE SEQUENCE DEPT_SEQ
    ...
    START WITH 1   -- 1부터 번호 증가
    NOMAXVALUE     -- 상한선 없음
    NOMINVALUE     -- 하한선 없음
    NOCYCLE        -- 번호 순환 없음
    CACHE 20       -- 번호 20개 미리 뽑아놓기 (번호를 미리 생성하지 않는 NOCACHE옵션을 사용함)
...
CREATE TABLE DEPARTMENT_T(
    ...
    CONSTRAINT PK_DEPERTMENT PRIMARY KEY(DEPT_NO)
);
...
/*
    ...
    UPDATE 테이블명 SET 업데이트할내용, 업데이트내용, ... WHERE 조건식 
*/


-- 1. DEPARTMENT_T에서 부서번호(DEPT_NO)가 3인 부서의 지역을 '인천'으로 수정하시오.
UPDATE DEPARTMENT_T 
   SET LOCATION = '인천' -- SET절의 등호(=)는 대입 연산자
 WHERE DEPT_NO = 3;      -- WHERE절의 등호(=)는 동등비교 연산자

-- 2. EMPLOYEE_T에서 부서번호(DEPART)가 2인 부서의 사원들의 기본급을 10% 인상하시오.
UPDATE EMPLOYEE_T
   SET SALARY = SALARY * 1.1
 WHERE DEPART = 2;
 
-- ROLLBACK
ROLLBACK; --이전 COMMIT 기준 이후로 실행 취소
...
-- 1. DEPARTMENT_T 에서 부서번호(DEPT_NO)가 3인 부서을 삭제하시오.(부서번호가 3인 사원은 없다. 참조 무결성에 영향을 미치지 않는다.)
DELETE 
FROM DEPARTMENT_T
WHERE DEPT_NO = 3;

-- 2. EMPLOYEE_T 에서 부서번호(DEPART)가 1, 4인 사원을 삭제하시오.
DELETE 
FROM EMPLOYEE_T
WHERE DEPART = 1 OR DEPART = 4; --WHERE DEPART IN(1,4)

-- 3. DEPARTMENT_T에서 부서번호(DEPT_NO)가 2인 부서를 삭제하시오. 
--(부서번호가 2인 사원이 있다. 부서가 없어지면 사원 정보가 찹조 무결성을 위배에 대비해서 ON DELETE SET NULL 옵션을 주었으므로 테이블 부서번호가 NULL로 함께 수정된다.)
DELETE 
FROM DEPARTMENT_T
WHERE DEPT_NO = 2;
