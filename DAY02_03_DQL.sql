/*
    ...
    2. 데이터 조회어
    3. 데이터(행, ROW)를 조회할때 사용한다.
    4. DQL을 실행해도 데이터베이스에 변화는 발생하지 않는다.
    5. 형식
        SELECT 조회할칼럼, 조회할칼럼, ...
        FROM 테이블명
        WHERE 조건식
        GROUP BY 그룹화할칼럼, 그룹화할칼럼, ... }같이 들어가야함
        HAVING 그룹조건식                        }
        ORDER BY 정렬할칼럼 ASC/DESC, 정렬할칼럼 ASC/DESC, ...
*/

-- 1. 부서 테이블의 모든 칼럼을 조회하시오.
SELECT DEPT_NO, DEPT_NAME, LOCATION      -- 권장방법
FROM DEPARTMENT_T;

SELECT *            -- 모든 칼럼을 의미하는 *, 실제 개발할 때는 사용 금지
FROM DEPARTMENT_T;

SELECT DEPARTMENT_T.DEPT_NO, DEPARTMENT_T.DEPT_NAME, DEPARTMENT_T.LOCATION     --다수의 칼럼에서 자료를 가져올 때 
FROM DEPARTMENT_T;              
...
SELECT DEPT_NO AS 부서번호  -- 칼럼의 별명(ALIAS)을 부서번호으로 지정
     , DEPT_NAME AS 부서명
     , LOCATION AS 위치
FROM DEPARTMENT_T;
...
SELECT LOCATION 
FROM DEPARTMENT_T
GROUP BY LOCATION;   -- 같은 로케이션을 묶어서
...
/*
    ...
    4. 칼럼 IS NULL
    5. 칼럼 IS NOT NULL
    6. 칼럼 LIKE 값  --값이 포함된 칼럼을 찾는다.
*/

-- 4. 부서 테이블에서 부서번호가 1인 부서 정보를 조회하시오. 
SELECT *
FROM DEPARTMENT_T
WHERE DEPT_NO = 1;            --부서번호는 유니크 하기 때문에 당연히 값은 1개 이하이다.


-- 5. 부서 테이블에서 지역이 '서울'인 부서 정보를 조회하시오.
SELECT *
FROM DEPARTMENT_T
WHERE LOCATION = '서울';     --로케이션은 유니크 하지 않기 때문에 결과가 2개 이상이 가능하다.

-- 6. 사원 테이블에서 기본급이 3000000이상인 사원을 조회하시오.
SELECT *
FROM EMPLOYEE_T
WHERE SALARY >= 3000000;

-- 7. 사원 테이블에서 기본급이 2000000에서 3000000인 사원을 조회하시오.
SELECT *
FROM EMPLOYEE_T
